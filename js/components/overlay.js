export function renderOverlay(state) {
  ...
  return `<div class="overlay-backdrop" data-action="close-overlay">
    <section class="overlay-panel" role="dialog" aria-modal="true" aria-label="${type}">
      <div class="overlay-head"><h2>${overlayTitle(type)}</h2><button class="icon-button" data-action="close-overlay">×</button></div>
      ${body}
    </section>
  </div>`;
}
...
function overlayBody(type, state) {
  ...
  if (type === 'risk') return `<div class="info-grid"><div class="info-cell"><small>POSITION</small><b>${state.market.position === 0 ? 'FLAT' : `${Math.abs(state.market.position).toFixed(2)} ${state.market.position > 0 ? 'LONG' : 'SHORT'}`}</b></div><div class="info-cell"><small>SESSION P&L</small><b class="${state.market.pnl >= 0 ? 'green':'red'}">${state.market.pnl.toFixed(3)} SOL</b></div><div class="info-cell"><small>MARKET DEPTH</small><b>${state.market.depth.toFixed(2)}</b></div><div class="info-cell"><small>LAST FLOW</small><b>${state.market.lastFlow >= 0 ? '+' : ''}${state.market.lastFlow.toFixed(2)}</b></div></div>`;
  if (type === 'phone') return `<div class="card"><div class="row"><div><b>NovaFund</b><div class="muted micro">Market invite · 2m ago</div></div><button class="btn">OPEN</button></div></div><div class="card"><div class="row"><div><b>WolfCapital</b><div class="muted micro">“Pit opens soon.”</div></div><button class="btn">REPLY</button></div></div>`;
  ...
  if (type === 'nextMarket') return `<div class="info-grid"><div class="info-cell"><small>ROOM</small><b>${state.markets.next.id}</b></div><div class="info-cell"><small>PLAYERS</small><b>${state.markets.next.players}/${state.markets.next.capacity}</b></div><div class="info-cell"><small>DURATION</small><b>${state.markets.next.durationMinutes} MIN</b></div><div class="info-cell"><small>BUY-IN</small><b>${state.markets.next.buyIn.toFixed(2)} SOL</b></div></div><button class="btn gold" data-view="market" style="width:100%;margin-top:10px">OPEN MARKET TERMINAL</button>`;
